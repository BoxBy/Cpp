...
unsigned char frmaePool[MaxFrames*sizeof(Frame)];

Frame* allFrames = 0;

bool allof[MaxFrames];
 // if alloc[i] is true with storage
 // cell is allocated
 ...
 void* Frame::operator new(size_t size){
     ...
     if(allFrames == 0){
         allFrames = reinterpret_cast<Frame*>(frmaePool);
...
int main(){
    ...
    try{
        ...
        for(int i = 0; i < 4; i++)
            ...
            //allocated Frames : the 1st
            //frame is already occupied
    } catch(string s){
        cerr << s << '\n';
        exit(EXIT_FAILURE);
    }
